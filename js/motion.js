...
(function () {
  ...
  function openFortuneCard() {
    ...
    // 随机拣一项（按页面：按钮墙→按钮；歌单页→歌曲；其它→歌单数据里的歌）
    ...
    let chosenSong = null;   // 歌单页卡片
    let chosenBtn = null;    // 按钮墙卡片
    let chosenName = '';
    let chosenArtist = '';
    let dataSong = null;     // 从歌曲数据抽出的真实歌（workbench/24点等无卡片页）

    if (soundBtns.length > 0) {
      // 按钮墙：抽按钮，显示按钮名
      chosenBtn = soundBtns[Math.floor(Math.random() * soundBtns.length)];
      chosenName = chosenBtn.querySelector('.sb-name')?.textContent?.trim() || '经典按钮';
      chosenArtist = '按钮墙 · 按键';
    } else if (songCards.length > 0) {
      // 歌单页：抽歌曲卡
      const card = songCards[Math.floor(Math.random() * songCards.length)];
      chosenSong = card;
      chosenName = card.querySelector('.song-title, .song-name, .song-name-text')?.textContent?.trim() || '经典曲目';
      chosenArtist = card.querySelector('.song-artist, .song-artist-text')?.textContent?.trim() || '小松绿 演唱';
    } else if (window.XSL_DATA && window.XSL_DATA.song_catalog && window.XSL_DATA.song_catalog.songs && window.XSL_DATA.song_catalog.songs.length) {
      // 无卡片页（工作台/24点）：从歌单数据抽真实歌（确保是歌单里有的歌）
      dataSong = window.XSL_DATA.song_catalog.songs[Math.floor(Math.random() * window.XSL_DATA.song_catalog.songs.length)];
      chosenName = (dataSong.display_song_name || dataSong.song_name || '').trim();
      chosenArtist = dataSong.artist || '小松绿 演唱';
    }
    ...
    playBtn.onclick = function () {
      ...
      // 按钮墙：直接点击抽中的那枚按钮（播放对应按钮）
      if (chosenBtn) {
        chosenBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
        setTimeout(() => chosenBtn.click(), 300);
        return;
      }
      // 歌单页：点击抽中歌曲卡的播放按钮（播放对应歌曲）
      if (chosenSong) {
        chosenSong.scrollIntoView({ behavior: 'smooth', block: 'center' });
        const playButton = chosenSong.querySelector('.song-play-btn') || chosenSong.querySelector('.sound-btn');
        setTimeout(() => playButton && playButton.click(), 300);
        return;
      }
      // 数据歌（工作台本地播 / 24点等跳转歌单自动播放）
      if (dataSong) {
        if (typeof window.playWorkshopSong === 'function') {
          window.playWorkshopSong(dataSong.display_song_name || dataSong.song_name);
        } else {
          window.location.href = `../index.html?play=${encodeURIComponent(dataSong.display_song_name || dataSong.song_name || '')}`;
        }
        return;
      }
      // 兜底：跳主歌单搜索
