...
(function () {
  ...
  const SEARCH_ENGINES = {
    ...
    songs: { name: '🎵 站内搜歌', url: 'https://viridis.love/?q=', placeholder: '在小松绿歌单中检索歌曲、歌手…', internal: true }
  };
  ...
  function initSearch() {
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        if (engine.internal) {
          window.location.href = `https://viridis.love/?q=${encodeURIComponent(query)}`;
        } else {
  ...
  function togglePlaySong() {
    ...
    if (playerState.isPlaying) {
      ...
      // 直接在 workshop 本地播放；音频路径相对 audio_index（如 assets/audio/...），
      ...
      if (audioRel) {
        ...
        showToast(`《${song.song_name}》暂未收录音频，建议前往完整歌单查看 🎵`);
      }
    }
  }
  ...
  // 供桌宠/好运歌签等从工作台直接本地播放指定歌曲（通过歌名匹配 playerState + audio_index）
  window.playWorkshopSong = function (name) {
    if (!name) { showToast('未指定歌曲'); return false; }
    try {
      let idx = playerState.songs.findIndex(s => (s.song_name || s.display_song_name || '') === name);
      if (idx < 0) idx = playerState.songs.findIndex(s => (s.song_name || '').includes(name) || (s.display_song_name || '').includes(name));
      if (idx < 0) { showToast(`未找到《${name}》，建议前往完整歌单`); return false; }
      playerState.currentIndex = idx;
      const song = playerState.songs[idx];
      renderPlayerSong(song);
      const rel = playerState.audioMap[song.row_key || song.song_name];
      if (rel) {
        playerState.audio.src = audioAbs(rel);
        playerState.audio.play().catch(() => showToast(`《${name}》音频暂不可用`));
      } else {
        showToast(`《${song.song_name}》暂未收录音频`);
      }
      return true;
    } catch (e) { console.warn('[playWorkshopSong]', e); return false; }
  };
